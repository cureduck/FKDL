...
public class CellGoodView : MonoBehaviour
{
    ...
    [SerializeField]
    private Localize itemName_txt;
    [SerializeField]
    private GameObject soldOutSign;
    [SerializeField]
    private GameObject discountSign;
    [SerializeField]
    ...
    private void OnPointEnter()
    {
        if (offer.Kind == Offer.OfferKind.Skill)
        {
            ...
            WindowManager.Instance.skillInfoPanel.Open(new SkillInfoPanel.Args02 { screenPosition = itemIcon.transform.position, skill = skill });
        }
        else if (offer.Kind == Offer.OfferKind.Potion)
        {
            Potion potion = PotionManager.Instance.GetById(offer.Id);
            WindowManager.Instance.simpleInfoItemPanel.Open(new SimpleInfoItemPanel.Args { title = potion.Id, describe = potion.Des, screenPosition = itemIcon.transform.position });
        }
        else if (offer.Kind == Offer.OfferKind.Key) 
        {
            string temp;
            ...
            WindowManager.Instance.simpleInfoItemPanel.Open(new SimpleInfoItemPanel.Args { title = temp, describe = "一把钥匙", screenPosition = itemIcon.transform.position });
        }
